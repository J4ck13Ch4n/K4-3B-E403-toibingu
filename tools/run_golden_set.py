...
def run():
    ...
    parser.add_argument('--cases', nargs='+', choices=[f'G{i:02}' for i in range(1, 21)],
                        help='Run selected regression cases; never report this subset as the full golden score.')
    args = parser.parse_args()
    selected = sorted(set(args.cases)) if args.cases else [f'G{i:02}' for i in range(1, 21)]
    document = (ROOT / 'eval/golden-set.md').read_text(encoding='utf-8')
    definitions = document.split('**Câu A')[0]
    rows = {line.split('|')[1].strip(): line.split('|')[3].strip()
            for line in definitions.splitlines() if line.startswith('| G')}
    ...
    report = {
        ...
        'scope': 'full_golden_set' if len(selected) == 20 else 'selected_regression',
        'selected_cases': selected, 'total_golden_cases': 20,
    ...
    (folder / 'golden-set-before-run.md').write_bytes((ROOT / 'eval/golden-set.md').read_bytes())
